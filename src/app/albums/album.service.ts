...
export class AlbumService {
  ...
  private albumsUrl = 'https://t20-label-default-rtdb.europe-west1.firebasedatabase.app/albums';
  private albumListsUrl = 'https://t20-label-default-rtdb.europe-west1.firebasedatabase.app/albumLists';
  ...
  getAlbums(): Observable<Album[]> {
    ...
    map(albums => {
      return albums.sort((a, b) => { return b.duration - a.duration });
      }))
  };
  ...
  searchAlbums(word: string): Observable<Album[]> {
    return this.http.get<Album[]>(this.albumsUrl + '.json').pipe(
      map(album => {
        return album.filter(album => album.title.toLowerCase().includes(word.toLowerCase()))}
      ))};
